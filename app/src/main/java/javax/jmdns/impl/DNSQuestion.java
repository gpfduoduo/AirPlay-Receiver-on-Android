...
public class DNSQuestion extends DNSEntry
{
            ...
            ServiceInfoImpl info)
    {
        if ((info != null) && info.isAnnounced())
        {
            ...
            {
                answers.addAll(jmDNSImpl.getLocalHost().answers(this.getRecordClass(),
                    DNSRecordClass.UNIQUE, DNSConstants.DNS_TTL));
                answers.addAll(info.answers(this.getRecordClass(), DNSRecordClass.UNIQUE,
    ...
    {
        DNS4Address(String name, DNSRecordType type, DNSRecordClass recordClass,
                boolean unique)
        {
            super(name, type, recordClass, unique);
        }
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers)
        {
            DNSRecord answer = jmDNSImpl.getLocalHost().getDNSAddressRecord(
                this.getRecordType(), DNSRecordClass.UNIQUE, DNSConstants.DNS_TTL);
            if (answer != null)
            {
                answers.add(answer);
            }
    ...
    {
        DNS6Address(String name, DNSRecordType type, DNSRecordClass recordClass,
                boolean unique)
        {
            super(name, type, recordClass, unique);
        }
    ...
    {
        HostInformation(String name, DNSRecordType type, DNSRecordClass recordClass,
                boolean unique)
        {
            super(name, type, recordClass, unique);
        }
    ...
    private static class Pointer extends DNSQuestion
    {
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers)
        {
            ...
            if (this.isServicesDiscoveryMetaQuery())
            {
                ...
                {
                    ServiceTypeEntry typeEntry = jmDNSImpl.getServiceTypes().get(
                        serviceType);
            ...
            else if (this.isReverseLookup())
            {
                ...
                if ((ipValue != null) && (ipValue.length() > 0))
                {
                    ...
                    String hostIPAddress = (address != null
                    ...
                    if (ipValue.equalsIgnoreCase(hostIPAddress))
                    {
                        if (this.isV4ReverseLookup())
                        {
                            answers.add(jmDNSImpl.getLocalHost()
                                    .getDNSReverseAddressRecord(DNSRecordType.TYPE_A,
                                            DNSRecordClass.NOT_UNIQUE, DNSConstants.DNS_TTL));
                        }
                        if (this.isV6ReverseLookup())
                        {
                            answers.add(jmDNSImpl.getLocalHost()
                                    .getDNSReverseAddressRecord(DNSRecordType.TYPE_AAAA,
                                            DNSRecordClass.NOT_UNIQUE, DNSConstants.DNS_TTL));
                        }
                    }
                }
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers)
        {
            ...
            if (jmDNSImpl.getServiceTypes().containsKey(loname))
            {
                DNSQuestion question = new Pointer(this.getName(),
                    DNSRecordType.TYPE_PTR, this.getRecordClass(), this.isUnique());
