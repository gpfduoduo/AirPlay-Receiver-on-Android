...
public final class DNSOutgoing extends DNSMessage
{
    ...
    public DNSOutgoing(int flags, boolean multicast, int senderUDPPayload)
    {
        ...
        _maxUDPPayload = (senderUDPPayload > 0
            ? senderUDPPayload
     ...
     *
     * @param destination Set a destination address a packet should be sent to
     *            (instead the default one). You could use null to unset the
     *            forced destination.
     */
    ...
    public static class MessageOutputStream extends ByteArrayOutputStream
    {
        ...
        void writeRecord(DNSRecord rec, long now)
        {
            ...
            writeShort(rec.getRecordClass().indexValue()
                | ((rec.isUnique() && _out.isMulticast())
                    ? DNSRecordClass.CLASS_UNIQUE
                    : 0));
            writeInt((now == 0) ? rec.getTTL() : rec.getRemainingTTL(now));

            // We need to take into account the 2 size bytes
            MessageOutputStream record = new MessageOutputStream(512, _out, _offset
                + this.size() + 2);
            rec.write(record);
            byte[] byteArray = record.toByteArray();
